variable "dev_internet_apim_resource_group_name" {}

variable "dev_internet_apim_location" {}

variable "dev_internet_apim_publisher_name" {}

variable "dev_internet_apim_publisher_email" {}

variable "dev_internet_apim_name" {
    type = string
}

variable "dev_internet_apim_sku_name" {
...
variable "dev_internet_apim_tags" {
  description = "(optional) User-Defined tags"
  type        = map(string)
  default     = {}
}

variable "dev_internet_apim_certificate" {
  type = list(object({
...
variable "dev_internet_apim_additional_location" {
  type = list(object({
    location  = string
  }))
  default = null
...
variable "dev_internet_apim_identity" {
  type = list(object({
    i_type  = string
  }))
  default = null
...
variable "dev_internet_apim_hostname_configuration" {
  type = list(object({
...
variable "dev_internet_apim_notification_sender_email" {
  type = string
  default = null
}

variable "dev_internet_apim_policy" {
  type = object({
    xml_content  = string
    xml_link     = string
  })
...
variable "dev_internet_apim_protocols" {
  type = object({
    enable_http2  = bool
  })
  default = null
...
variable "dev_internet_apim_security" {
  type = object({
...
variable "dev_internet_apim_sign_in" {
  type = object({
    enabled  = bool
  })
  default = null
...
variable "dev_internet_apim_sign_up" {
  type = object({
    enabled  = bool
...
variable "dev_internet_apim_diag_object" {
