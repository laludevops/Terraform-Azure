variable "dev_backend_redis_cache_name" {
    type = string
}

variable "dev_backend_redis_cache_resource_group_name" {
    type = string
}

variable "dev_backend_redis_cache_location" {
    type = string
}


variable "dev_backend_redis_cache_diag_object" {
...
variable "dev_backend_redis_cache_tags" {
  description = "(optional) User-Defined tags"
  type        = map(string)
  default     = {}
}

variable "dev_backend_redis_cache_redis_cache" {
...
variable "dev_backend_redis_cache_firewall_name" {
  description = "(Required) Specify the name for firewall"
  type        = string
}

variable "dev_backend_redis_cache_start_ip" {
  description = "start ip address range for firewall"
  type        = string
}

variable "dev_backend_redis_cache_end_ip" {
  description = "End ip address range for firewall"
  type        = string
}

variable "dev_backend_redis_cache_standard_redis_cache" {
    description = "(Required) Specify the naming convention information to the resource."
    type =  object({      
...
variable "dev_backend_redis_cache_premium_redis_cache" {
...
variable "dev_backend_redis_cache_patch_schedule" {
    type = object(
         {
              day_of_week      = string
